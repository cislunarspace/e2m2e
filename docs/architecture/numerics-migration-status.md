...
- **pipeline / catalog 编排** 是串联与结果组装；数值段由 #464/#466
  下沉后自然变薄，不单独迁编排层。
- 数值主链：QF↔CM 已下沉（#465）；多项式核 / 中心流形见迁移中 #464/#466，
...
- **粒度**：登记粒度到文件/路径。同一模块可拆多条（如 `algorithm/design`：
  ...
  CR3BP Hamiltonian/H→QF/QF↔CM 已下沉，多项式核/中心流形迁移中
  #464/#466，符号与 NAFF/编排有意留 Python），以速查表路径为准。

...
其中，部分计算功能还由 Python 执行（NLP、NSGA-II 等），正在逐步迁移至 Rust 计算核心。延拓与低推力直接法的数值评估内核已下沉，Python 保留外层编排和参照路径。时空系统和常量模块还需要进一步审查（三条帧转换路径并存、时间转换责任链）。Python 编排模块架构还不够稳定（打靶双路径、transfer 目录数值残留），需要进一步审查和进行系统设计。

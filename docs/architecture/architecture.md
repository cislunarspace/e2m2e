...
其中，部分计算功能还由 Python 执行（延拓、NLP、NSGA-II、低推力打靶等），正在逐步迁移至 Rust 计算核心。时空系统和常量模块还需要进一步审查（三条帧转换路径并存、时间转换责任链）。Python 编排模块架构还不够稳定（打靶双路径、transfer 目录数值残留），需要进一步审查和进行系统设计。

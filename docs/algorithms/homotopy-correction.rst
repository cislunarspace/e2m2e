...
- **同伦过渡 / homotopy transition**：在动力学参数空间内构造一族
  连续插值模型 ``a(lambda) = a_base + lambda * (a_full - a_base)``，
  从 ``lambda=0``（base 模型）平滑过渡到 ``lambda=1``（full 模型），
  用以扩大多重打靶法的收敛盆地。
- **base 模型 / base bodies**：星历动力学中显式参与引力计算的天体子集。
  MVP 默认 ``["EARTH", "MOON"]``（不含太阳）。
- **full 模型 / full bodies**：星历动力学的完整天体集合。
...
- ``dynamics``：完整的 ``EphemerisDynamics`` 实例，其 ``system.bodies``
  即 full bodies 集合。
...
- :mod:`tests.algorithms.test_homotopy_two_level`：
  two_level 路径的 delegation、聚合语义、参数校验。
- :mod:`tests.algorithms.test_homotopy_ephemeris_integration`：

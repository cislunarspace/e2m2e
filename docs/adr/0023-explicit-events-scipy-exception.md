# ADR 0023: SciPy propagation exception for explicit event inputs / 显式事件输入的 SciPy 传播例外

[English](#adr-0023-scipy-propagation-exception-for-explicit-event-inputs) | [简体中文](#中文)
...
显式事件调用继续获得 SciPy 的事件语义和结果字段；普通传播保持 Rust 数值路径和缺失扩展的显式失败。两种场景的分派原因可由输入参数直接判断，不再把扩展缺失与事件例外混为一谈。

### 修订（2026-08-23）

本篇的分派规则经 ADR 0020 决策 4 修订：传入 `events` 不再自动分派 SciPy，事件路径要求调用者显式给出 `backend='scipy'` 或 `backend='rust'`（后者走 Rust `solve_ivp_events`）；传 `events` 而不传 `backend` 直接抛 `ValueError`。结果一节所述的 SciPy 事件语义只在 `backend='scipy'` 时成立。

...
**设计文档**：`docs/plans/transfer-grid-search-rust.md`
...
三档并行度统一 `n_workers=4`、每档每 backend 跑 3 次取中位 wall-time（48 核机器，CR3BP 地月，DOP853 rtol=atol=1e-9）。完整配置与脚本见 `scripts/benchmark_transfer_search.py` 与 `docs/plans/transfer-grid-search-rust-benchmark.md`。

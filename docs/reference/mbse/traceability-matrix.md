...
|---------|------|------|--------|----------|----------|----------|
| REQ-001 | 状态向量顺序 | interface | shall | test | orbit, dynamics | test_orbit.py, test_dynamics_contract.py |
| REQ-002 | 传播结果 states 形状 | interface | shall | test | dynamics, ephemeris_dynamics | test_dynamics_contract.py, test_ephemeris_dynamics_legacy.py |
| REQ-003 | Jacobi 常数漂移容限 | performance | shall | test | dynamics | test_cr3bp_model.py |
| REQ-004 | STM 解析 Jacobian | functional | shall | test | dynamics | test_cr3bp_variational.py |
| REQ-005 | Dynamics 子类调用 super().__init__() | interface | shall | inspection | dynamics, ephemeris_dynamics | test_ephemeris_dynamics_legacy.py |
| REQ-006 | 坐标变换互逆一致 | functional | shall | test | coordinate | test_coordinate.py, test_coordinate.py |
| REQ-010 | 平动点位置精度 | performance | shall | test | system | test_cr3bp_system.py |
| REQ-011 | 特征尺度设置前置条件 | functional | should | test | system | test_cr3bp_system.py |
| REQ-012 | 积分容差默认值 | performance | shall | inspection | dynamics | test_dynamics_contract.py |
...
| REQ-025 | EphemerisDynamics 统一接口 | interface | shall | test | ephemeris_dynamics | test_ephemeris_dynamics_legacy.py |
| REQ-026 | EphemerisDynamics 自适应步长 | functional | should | test | ephemeris_dynamics | test_ephemeris_dynamics_legacy.py |

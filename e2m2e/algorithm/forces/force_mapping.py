...
- 基础模型（常开）：地球质点 ``PointMassGravity(EARTH)`` （状态以地心为
  ...
  间接项，无需另补）。
- ``earth_nonspherical=1``：地球中心项换成
  ``GravityField(EARTH, degree=order=earth_degree)`` （球谐含 degree=0 中心项，
  不重复计质点）。
- ``moon_nonspherical=1``：月球中心项换成
  ``GravityField(MOON, degree=order=moon_degree)`` + ``IndirectTerm(MOON)`` （球谐只算直接
  引力，地心加速系需单补间接项）。

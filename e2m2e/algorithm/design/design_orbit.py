...
相位零点按黄金样本标定——Halo/NRHO 在 y=0 穿越点，DRO 在远侧
x 轴穿越点（e2m2e 的 DRO 参考状态为近侧穿越点，内部偏移半周期）。
DRO 振幅取一个周期内距月距离最小/最大值的均值（同按黄金样本标定）。

已知系统差（实测标定）：

- 原始输出 GCRS，e2m2e 在 ICRF（J2000）下传播，frame bias（~23 mas）
...
def _build_ephemeris_table(
    ...
    地月会合系为地心归一（月球在 +x 单位距离处；依据：黄金样本中 DRO
    轨道在该约定下关于 x=1 近似对称）；内部转换器输出质心归一（月球在
    1-mu），x 分量加 mu 平移对齐。
    """
...
def design_orbit(
    ...
    *,
    ...
    # 相位零点约定：Halo/NRHO 的参考状态（y=0 穿越点）与黄金样本的
    # phase=0 起点一致；DRO 的参考状态是近侧穿越点，而相位零点在远侧
    # 穿越点（黄金样本标定：黄金样本 phase=0.5001 的 DRO 首行恰在近侧穿越点，
    # 距月取最小值），差半个周期

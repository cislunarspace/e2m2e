...
class DROTRONLPOptimizer:
    ...
    def get_arrival_state_at_t_ins(
        self,
        t_ins: float
    ) -> Tuple[np.ndarray, np.ndarray]:
        """获取目标轨道上t_ins对应的状态
    ...
    def constraint_velocity_parallel(
        self,
        y: np.ndarray
    ) -> float:
    ...
    def optimize(
        ...
        if use_relaxed_velocity_constraint:
            ...
            constraints.append({
                'type': 'ineq',
                'fun': lambda y: cos_theta_max - self._compute_cos_angle(y)
            })
        else:
            constraints.append({
                'type': 'eq',
                'fun': self.constraint_velocity_parallel
            })
    ...
    def _build_result(
        ...
        if success:
            ...
            if use_relaxed_constraint:
                violation['velocity'] = max(0, np.cos(velocity_angle_constraint) - self._compute_cos_angle(variables.to_array()))
            else:

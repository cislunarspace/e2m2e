...
``SPICEManager.get_body_position`` / ``get_body_state``（spice.py:246-261,
228-244）——无缓存、每次跨 Python↔C 边界查 SPICE。30 天弧段、10 个天体、
每步多次查询，累积开销巨大。

本模块在积分前用 SPICE 在均匀网格上预采样所有相关天体的 (R, V)，建
``scipy.interpolate.CubicSpline``（C² 连续）。之后 SPICEManager 的
``get_body_position`` / ``get_body_state`` 走 spline 插值（纯数值），
不再跨边界。

为什么用三次样条而不是线性插值：线性插值 C⁰ 连续，网格点处导数不连续，
会让自适应积分器（PD45/PD78）的误差估计器不断缩步长（实测 93 倍 RHS
调用）。三次样条 C² 连续消除此问题（经验来自 qiao 仓库
``Python/crtbp/subfunction/ephfunc/ephem_table.py:10-12``）。

与 qiao ``ephem_table.py`` 的区别：qiao 存 12 列 (R_EM,V_EM,R_ES,V_ES)
并为 rho 会合系推导 A_EM/J_EM/C/Cdot/Cdotdot；e2m2e 的 ForceModel 在
ECI 系，只需天体位置/速度本身，不推导旋转矩阵。故本模块更简化：每个
天体一对 CubicSpline（位置、速度）。
"""
...
# 默认网格步长（秒）。qiao 经验值每小时一点（precomputeEphTable.m:31，
# ephem_table.rs:23 的 dt=0.01 TU ≈ 1 小时）。月球轨道周期 ~27 天，1 小时
# 间隔下三次样条位置误差 < 1 km。
DEFAULT_DT_SECONDS: float = 3600.0

# 两端外推 margin（倍数）。积分器可能在 [et_start, et_end] 边界附近因步长
# 越界查到端点外，留 margin 避免外推。qiao run_experiment.py:161 惯例。
...
def build_ephem_cache(
    ...
    *,
    ...
    Args:
        spice: 已加载星历内核的 SPICEManager。
        bodies: 要缓存的天体名列表（如 ``["MOON", "SUN", "JUPITER"]``）。
        et_start, et_end: 积分区间的起止 ET 秒。
        dt: 网格步长（秒），默认 1 小时。
        frame: 坐标系，默认 ``"J2000"``。
        observer: 观察者天体，默认 ``"EARTH"``（地心）。
        margin_steps: 两端外推 margin 的步数，默认 5。

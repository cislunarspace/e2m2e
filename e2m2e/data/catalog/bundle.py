"""基线分发包（ADR 0045 决策 8；ADR 0047 出包）：v1 族束传输格式 → v2
成员记录展开。

族束（Release 资产内、仓库回归夹具同形态）是分发与压缩的传输单元，
不是轨道记录：JSON 元数据 + ``members`` 成员表 + ``cr3bp/members/``
段数组，冻结在 v1 布局。显式导入（``baseline.py``）把每束展开为
逐成员的 v2 轨道记录——库内只有一种记录形态。

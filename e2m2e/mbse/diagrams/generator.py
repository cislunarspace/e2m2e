...
class DiagramGenerator:
    ...
    def generate_traceability_matrix(self) -> str:
        ...
        lines = [
            "| Requirement ID | Title | Category | Priority | Verification |"
            " Linked code | Linked tests |",
            "|---------|------|------|--------|----------|----------|----------|",
        ]
    ...
    def write_document(
        ...
        """将受管 Markdown 文档写入指定位置（英文）。"""
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        preamble = (
            intro + "\n\nManaged artifact: regenerated by `scripts/generate_mbse_diagrams.py`;"
            " do not hand-edit.\n\n"
            if intro
            else ""
        )
    ...
    def generate_all(self, output_dir: str) -> list[str]:
        ...
        for layer in ARCHITECTURE_LAYERS:
            ...
            title, intro = layer_titles[layer]
            self.write_diagram(title, bdd_content, path, intro)

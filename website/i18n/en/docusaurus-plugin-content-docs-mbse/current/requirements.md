```mermaid
requirementDiagram
    requirement REQ_001 {
        title: State vector order
        type: interface
        risk: shall
    }
    e2m2e_core_orbit -satisfies-> REQ_001
    e2m2e_core_dynamics -satisfies-> REQ_001
    requirement REQ_002 {
        title: Propagation result states shape
        type: interface
        risk: shall
    }
    e2m2e_core_dynamics -satisfies-> REQ_002
    e2m2e_core_ephemeris_dynamics -satisfies-> REQ_002
    requirement REQ_003 {
        title: Jacobi constant drift tolerance
        type: performance
        risk: shall
    }
    e2m2e_core_dynamics -satisfies-> REQ_003
    requirement REQ_004 {
        title: STM analytical Jacobian
        type: functional
        risk: shall
    }
    e2m2e_core_dynamics -satisfies-> REQ_004
    requirement REQ_005 {
        title: Dynamics subclass calls super().__init__()
        type: interface
        risk: shall
    }
    e2m2e_core_dynamics -satisfies-> REQ_005
    e2m2e_core_ephemeris_dynamics -satisfies-> REQ_005
    requirement REQ_006 {
        title: Coordinate transformation inverse consistency
        type: functional
        risk: shall
    }
    e2m2e_core_coordinate -satisfies-> REQ_006
    requirement REQ_010 {
        title: Libration point position accuracy
        type: performance
        risk: shall
    }
    e2m2e_core_system -satisfies-> REQ_010
    requirement REQ_011 {
        title: Characteristic scales setup precondition
        type: functional
        risk: should
    }
    e2m2e_core_system -satisfies-> REQ_011
    requirement REQ_012 {
        title: Integration tolerance defaults
        type: performance
        risk: shall
    }
    e2m2e_core_dynamics -satisfies-> REQ_012
    requirement REQ_020 {
        title: Orbit serialization compatibility
        type: interface
        risk: shall
    }
    e2m2e_core_orbit -satisfies-> REQ_020
    requirement REQ_021 {
        title: Orbit period estimation
        type: functional
        risk: should
    }
    e2m2e_core_orbit -satisfies-> REQ_021
    requirement REQ_022 {
        title: OrbitFamily aggregation
        type: functional
        risk: shall
    }
    e2m2e_core_orbit -satisfies-> REQ_022
    requirement REQ_025 {
        title: EphemerisDynamics unified interface
        type: interface
        risk: shall
    }
    e2m2e_core_ephemeris_dynamics -satisfies-> REQ_025
    requirement REQ_026 {
        title: EphemerisDynamics adaptive step size
        type: functional
        risk: should
    }
    e2m2e_core_ephemeris_dynamics -satisfies-> REQ_026
    requirement REQ_100 {
        title: Differential correction converges within 50 iterations
        type: performance
        risk: shall
    }
    e2m2e_algorithms_differential_correction -satisfies-> REQ_100
    requirement REQ_101 {
        title: Convergence tolerance default 1e-12
        type: performance
        risk: shall
    }
    e2m2e_algorithms_differential_correction -satisfies-> REQ_101
    requirement REQ_102 {
        title: Strategy pattern separates configuration and iteration
        type: interface
        risk: should
    }
    e2m2e_algorithms_strategies -satisfies-> REQ_102
    e2m2e_algorithms_differential_correction -satisfies-> REQ_102
    requirement REQ_103 {
        title: Continuation does not duplicate CR3BP physics
        type: interface
        risk: shall
    }
    e2m2e_algorithms_continuation -satisfies-> REQ_103
    requirement REQ_104 {
        title: Algorithm layer STM analytical computation
        type: functional
        risk: shall
    }
    e2m2e_algorithms_differential_correction -satisfies-> REQ_104
    requirement REQ_105 {
        title: Richardson third-order approximation accuracy
        type: functional
        risk: should
    }
    e2m2e_algorithms_differential_correction -satisfies-> REQ_105
    requirement REQ_110 {
        title: Stability index satisfies v1*v2 = 1
        type: performance
        risk: should
    }
    e2m2e_algorithms_stability -satisfies-> REQ_110
    requirement REQ_111 {
        title: MultipleShooting parallel propagation
        type: functional
        risk: should
    }
    e2m2e_algorithms_multiple_shooting -satisfies-> REQ_111
    requirement REQ_112 {
        title: Continuation step size adaptation
        type: functional
        risk: should
    }
    e2m2e_algorithms_continuation -satisfies-> REQ_112
    requirement REQ_113 {
        title: Pseudo-arclength continuation tangent vector computation
        type: functional
        risk: shall
    }
    e2m2e_algorithms_continuation -satisfies-> REQ_113
```

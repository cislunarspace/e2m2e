...
def test_module_does_not_export_optimizer_adapters():
    ...
    assert not hasattr(e2m2e.transfer, "TransferOptimizer")
    assert not hasattr(e2m2e.transfer, "SciPyTransferOptimizer")
    assert not hasattr(e2m2e.transfer, "COPTTransferOptimizer")

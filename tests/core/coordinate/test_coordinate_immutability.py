"""守门员测试:扫描坐标模块内不应有 Axes / Origin 属性的运行时 mutate。
...
def test_no_axes_or_origin_attribute_mutation_in_coordinate():
    ...
    扫描范围:``e2m2e/algorithm/coordinate/``(坐标转换算法)。其他模块允许
    出现局部变量名为 ``axes`` / ``origin`` 的常规用法,本规则不覆盖。
    """
    coord_root = Path(__file__).resolve().parents[3] / "e2m2e" / "algorithm" / "coordinate"
    ...
    for py in sorted(coord_root.rglob("*.py")):
        for lineno, line in enumerate(py.read_text(encoding="utf-8").splitlines(), start=1):
            stripped = line.lstrip()
            if stripped.startswith("#"):
                continue
            if pattern.search(line):
                offenders.append((str(py.relative_to(coord_root)), lineno, line.strip()))

    assert not offenders, (
        "坐标模块检测到 Axes/Origin 属性的运行时 mutate,违反 immutability 规范:\n"

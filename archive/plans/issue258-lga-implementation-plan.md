...
**差异说明**：Zhang/Belbruno/Gómez 的 Ω 比 Parker 的 U 多了常数项 ½μ(1-μ)。两者使用**相同的质心旋转系**（原点在地月质心），差异纯粹是伪势定义的约定选择——是否包含常数项 ½μ(1-μ)。Gómez 2001 在同一本书中明确记录了两种约定（Ch.1 §1.1.2 Convention A 无常数、Ch.2 §2.1.1 Convention B 有常数），坐标系完全相同。常数项在梯度运算中消失（不影响运动方程），只改变 Jacobi 常数的绝对值。**对守恒量验证无影响**——飞越前后 C 的差值在两种约定下相同。
